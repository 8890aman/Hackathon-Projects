...
const Sidebar = ({ isCollapsed, toggleCollapse, onSectionChange }) => {
  ...
  // Update sidebar height based on screen size
  useEffect(() => {
    const handleResize = () => {
      const height = window.innerHeight;
      // For laptops with small height, make sure it doesn't overflow
      if (height < 800) {
        setSidebarHeight(Math.min(height, 700) + 'px');
      } else {
  ...
  return (
    <div className="fixed top-0 right-0 bottom-0 hidden md:block" style={{ 
        ...
        <div className="bg-[#FF0000] rounded-l-2xl overflow-hidden h-full border-4 border-[#CC0000] shadow-lg flex flex-col relative">
          ...
          <div className="h-16 bg-[#FF0000] relative">
            ...
            {miniPlayerVisible && isPlaying && !showMusic && (
              <div className="absolute top-1 right-4 z-50 w-[150px] sm:w-[170px] bg-[#2a6fa8] rounded-lg shadow-md border-2 border-[#85DDFF]">
                <div className="flex items-center p-1.5 relative">
                  <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#1a4971] flex items-center justify-center mr-1.5 sm:mr-2 border border-[#85DDFF]">
                    {isPlaying ? (
                      ...
                      <MusicalNoteIcon className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-[#85DDFF]" />
                    )}
                  </div>
                  <div className="flex-1 overflow-hidden cursor-pointer" onClick={() => setShowMusic(true)}>
                    <Typography className="font-pixel text-[5px] sm:text-[6px] text-white truncate">
                      {isPlaying ? "Now Playing:" : "Paused:"}
                    </Typography>
                    <Typography className="font-pixel text-[7px] sm:text-[8px] text-[#85DDFF] font-bold truncate">
                      {musicTracks[selectedTrackIndex].name}
                    </Typography>
                  </div>
                  <div className="flex items-center">
                    <button 
                      className="w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center bg-[#85DDFF]/20 hover:bg-[#85DDFF]/40 mr-1"
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      {isPlaying ? (
                        ...
                        <div className="w-0 h-0 border-t-[1.5px] sm:border-t-[2px] border-t-transparent border-b-[1.5px] sm:border-b-[2px] border-b-transparent border-l-[3px] sm:border-l-[4px] border-l-white ml-0.5"></div>
                      )}
                    </button>
                    <button 
                      className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center rounded-full bg-[#85DDFF]/20 hover:bg-[#85DDFF]/40"
                      onClick={closeMiniPlayer}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-2 w-2 sm:h-2.5 sm:w-2.5 text-white" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div className="bg-[#232323] h-full p-1">
                ...
                {showMusic ? (
                  ...
                  <div className="w-full h-full flex flex-col bg-gradient-to-b from-[#1a4971] to-[#2a6fa8] rounded-sm overflow-hidden">
                    ...
                    {/* Playback controls */}
                    <div className="bg-[#1a4971] border-t border-[#85DDFF]/50 p-1.5 sm:p-2">
                      <div className="flex justify-between items-center">
                        <button
                          ...
                          className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-[#2a6fa8]/70 flex items-center justify-center border border-[#85DDFF]/30 hover:bg-[#3298cb] transition-colors duration-200"
                        >
                          <div className="w-0 h-0 border-t-[3px] sm:border-t-[4px] border-t-transparent border-b-[3px] sm:border-b-[4px] border-b-transparent border-r-[5px] sm:border-r-[6px] border-r-white ml-[-2px]"></div>
                        </button>
                        ...
                        >
                          {isPlaying ? (
                            <div className="w-3 h-4 sm:w-4 sm:h-5 bg-white rounded-sm"></div>
                          ) : (
                            <div className="w-0 h-0 border-t-[6px] sm:border-t-[8px] border-t-transparent border-b-[6px] sm:border-b-[8px] border-b-transparent border-l-[10px] sm:border-l-[12px] border-l-white ml-1"></div>
                          )}
                        </button>
                        ...
                        <button
                          ...
                          className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-[#2a6fa8]/70 flex items-center justify-center border border-[#85DDFF]/30 hover:bg-[#3298cb] transition-colors duration-200"
                        >
                          <div className="w-0 h-0 border-t-[3px] sm:border-t-[4px] border-t-transparent border-b-[3px] sm:border-b-[4px] border-b-transparent border-l-[5px] sm:border-l-[6px] border-l-white mr-[-2px]"></div>
                        </button>
                      </div>
                    ...
                    {/* Header Bar with Back Button */}
                      ...
                      {/* Pokémon Image Area */}
                      <div className="bg-[#FFFFFF] flex-1 flex items-center justify-center p-2 relative" 
                        style={{ 
                          minHeight: `${Math.min(220, window.innerHeight * 0.3)}px`,
                      ...
                      <img 
                        ...
                        alt={currentCreature.name} 
                          className="w-3/4 h-3/4 sm:w-4/5 sm:h-4/5 object-contain z-10 max-h-[200px]" 
                      />
                    </div>
                  ...
                  <div className="w-full h-full flex flex-col bg-[#2a6fa8] rounded-sm">
                    ...
                    <div className="flex-1 p-1 sm:p-2 overflow-auto">
                      {menuItems.map((item, index) => (
                        <div 
                          key={item.id}
                          className={`mb-1.5 sm:mb-2 p-1.5 sm:p-2 rounded-lg transition-all duration-200 ${
                        ...
                        >
                          <div className="flex items-center mb-0.5 sm:mb-1">
                            <item.icon className="h-3 w-3 sm:h-4 sm:w-4 mr-1.5 sm:mr-2 text-white" />
                            <Typography className="font-pixel text-[9px] sm:text-[10px] text-white font-bold">
                              {item.label}
                            </Typography>
                            {selectedMenuIndex === index && (
                              <div className="ml-2 w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-white animate-pulse"></div>
                            )}
                          </div>
                          <Typography className="font-pixel text-[7px] sm:text-[8px] text-white/80">
                            {item.description}
                          </Typography>
                        </div>
                      ))}
          ...
          {/* Control Pad Section */}
          <div className="px-6 pb-4 flex flex-col gap-2 sm:gap-3">
            {/* D-Pad */}
            <div className="flex justify-center mb-1 sm:mb-2">
              <div className="w-28 h-28 sm:w-36 sm:h-36 relative">
                {/* Center Circle */}
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#222] z-10"></div>
                
                {/* Up Button */}
              <button 
                  className="absolute top-1 left-1/2 transform -translate-x-1/2 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center z-20 transition-all duration-150"
                onClick={handleDPadUp}
              >
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-sm bg-[#222] flex items-center justify-center hover:bg-[#333] active:bg-[#111] transition-colors duration-150 border border-transparent hover:border-white">
                    <div className="w-0 h-0 border-l-[4px] sm:border-l-[5px] border-l-transparent border-r-[4px] sm:border-r-[5px] border-r-transparent border-b-[6px] sm:border-b-[8px] border-b-white"></div>
                  </div>
              </button>
              
                {/* Right Button */}
                <button 
                  className="absolute top-1/2 right-1 transform -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center z-20 transition-all duration-150"
                  onClick={handleDPadRight}
                >
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-sm bg-[#222] flex items-center justify-center hover:bg-[#333] active:bg-[#111] transition-colors duration-150 border border-transparent hover:border-white">
                    <div className="w-0 h-0 border-t-[4px] sm:border-t-[5px] border-t-transparent border-b-[4px] sm:border-b-[5px] border-b-transparent border-l-[6px] sm:border-l-[8px] border-l-white"></div>
                  </div>
                </button>
                
                {/* Down Button */}
                <button 
                  className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center z-20 transition-all duration-150"
                  onClick={handleDPadDown}
                >
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-sm bg-[#222] flex items-center justify-center hover:bg-[#333] active:bg-[#111] transition-colors duration-150 border border-transparent hover:border-white">
                    <div className="w-0 h-0 border-l-[4px] sm:border-l-[5px] border-l-transparent border-r-[4px] sm:border-r-[5px] border-r-transparent border-t-[6px] sm:border-t-[8px] border-t-white"></div>
                  </div>
                </button>
                
                {/* Left Button */}
                <button 
                  className="absolute top-1/2 left-1 transform -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center z-20 transition-all duration-150"
                  onClick={handleDPadLeft}
                >
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-sm bg-[#222] flex items-center justify-center hover:bg-[#333] active:bg-[#111] transition-colors duration-150 border border-transparent hover:border-white">
                    <div className="w-0 h-0 border-t-[4px] sm:border-t-[5px] border-t-transparent border-b-[4px] sm:border-b-[5px] border-b-transparent border-r-[6px] sm:border-r-[8px] border-r-white"></div>
                  </div>
                </button>
              </div>
            </div>
            ...
            <div className="flex justify-around">
              ...
              <button 
                className="w-12 h-12 sm:w-14 sm:h-14 bg-[#222222] hover:bg-[#333333] active:bg-[#111111] rounded-full shadow-md border-2 border-[#555555] flex items-center justify-center transition-transform duration-150 transform active:scale-95 hover:shadow-lg hover:border-white"
                onClick={handleDPadSelect}
              >
                <div className="flex flex-col items-center justify-center">
                  <div className="w-5 h-5 flex items-center justify-center bg-[#444444] rounded-full mb-0.5 border border-[#666666]">
                    <span className="text-white text-xs font-bold">A</span>
                  </div>
                  <Typography className="font-pixel text-[8px] text-white font-bold">
                    CHOOSE
                  </Typography>
                </div>
              </button>
            ...
            <div className="flex justify-between items-center mt-1">
              ...
              <div className="flex space-x-2">
                <button
                  className="w-10 h-4 bg-[#444444] hover:bg-[#555555] active:bg-[#333333] rounded shadow-sm border border-[#222222] flex items-center justify-center hover:border-white transition-colors duration-200"
                  onClick={toggleMusicEnabled}
                >
                ...
                <button
                  className="w-10 h-4 bg-[#FF5252] hover:bg-[#FF6E6E] active:bg-[#D64545] rounded shadow-sm border border-[#CC0000] flex items-center justify-center hover:border-white transition-colors duration-200"
                >
                  <Typography className="font-pixel text-[6px] text-white">
                    POWER
              </Typography>
